...
function buildDefaultFields() {
  const defaults = {};
  ALL_FIELDS.forEach(f => { defaults[f.key] = true; });
  return defaults;
}
...
export function SettingsProvider({ children }) {
  const [settings, setSettingsRaw] = useState(() => {
    ...
    parsed.harvestLoadForm.fields = mergeFields(parsed.harvestLoadForm.fields, ALL_FIELDS);
  ...
  useEffect(() => {
    setSettingsRaw(current => {
      ...
      patched.harvestLoadForm.fields = mergeFields(patched.harvestLoadForm.fields, ALL_FIELDS);
  ...
  const setSettings = updater => {
    setSettingsRaw(prev => {
      ...
      next.harvestLoadForm.fields = mergeFields(next.harvestLoadForm.fields, ALL_FIELDS);
