...
export default function BlendsScreen() {
  ...
  const filtered = blends
    ...
    .sort((a, b) => {
      ...
      if (sortCol === 'fg') { v1 = parseFloat(v1); v2 = parseFloat(v2); }
  ...
  return (
    <div className="blendsscreen-root">
      <div className="header-bar">
        <button className="header-back-btn" onClick={handleBack}>← Back</button>
        <h2 className="header-title">Blends</h2>
        <button className="nav-btn" onClick={handleAdd}>+ Add Blend</button>
      </div>
      ...
      <div className="card blends-card">
        <div className="blendsscreen-searchbar">
          ...
          />
          <button className="nav-btn" onClick={() => setSearch('')}>Clear</button>
        </div>
            ...
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort('date_created')}>
                  Date Created {sortCol === 'date_created' && (sortDir === 'asc' ? '▲' : '▼')}
                </th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(blend => (
                ...
                >
                  ...
                  <td>{blend.bulk}</td>
                  <td>{blend.fg}</td>
                  <td>{blend.date_created && new Date(blend.date_created).toLocaleDateString()}</td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={4} style={{ textAlign: "center" }}>No blends found.</td>
                </tr>
              )}
            </tbody>
          </table>
